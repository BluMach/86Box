...
nvr_at_init(const device_t *info)
{
    ...
    /* Factory state expected by the M250/M250 E Phoenix diagnostics. Both
     * machines use the same NVR layout: one 3.5-inch floppy, 80x25 colour,
     * no coprocessor and 640 KiB base plus 384 KiB extended memory in the
     * conservative 1 MiB profile. Save immediately because the firmware may
     * not touch every field before a subsequent warm reset. */
    if (nvr->is_new &&
