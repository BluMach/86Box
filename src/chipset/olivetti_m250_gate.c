...
#define ENABLE_OLIVETTI_M250_GATE_LOG 1
...
olivetti_m250_gate_set_e_variant(int on)
{
    ...
    if (current_m250_dev != NULL) {
        ...
        /* Re-register the IO handler set: remove 0x6B if we are
           switching to the non-E variant, add it if we are
           switching to the E variant. */
